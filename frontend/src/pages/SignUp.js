...
const SignUp = () => {
  // for navigating to login once signup is done
  ...
  // for handling the image uploaded for profile photo
  const handleUploadProfileImage = async (e)=>{
    // since there is promise involved so make it async
  ...
  const handleSubmit = async(e)=>{
    ...
    if(firstName && email && password && confirmPassword){
      if(password === confirmPassword){
        ...
        // now when we will be clicking signup the data will get appeared
        // alert(dataResponse.message)
        toast(dataResponse.message)
        if(dataResponse.alert){
          // alert is passed in backend , only when new id registered.... 
          navigate("/login")
        }
      }
      else{
        alert("pls check password once again!!")
      }
    }
    else{
      ...
      <div className="w-full max-w-sm bg-white m-auto flex  flex-col p-4 rounded ">
        ...
        <form className="w-full py-3 flex flex-col" onSubmit={handleSubmit}>
          ...
          <div className="flex px-2 py-1 bg-slate-200  rounded mt-1 mb-2 focus-within:outline focus-within:outline-blue-300"> 
            ...
            {/* we also want to have hide and show icon password */}
          ...
          <div className="flex px-2 py-1 bg-slate-200  rounded mt-1 mb-2 focus-within:outline focus-within:outline-blue-300"> 
            ...
            <span className="flex text-xl cursor-pointer" onClick={handleShowConfirmPassword}>
              {showConfirmPassword ? <BiShow />:
              <BiHide />}
            </span>
          </div>

          <button className=" w-full max-w-[150px] m-auto bg-red-500 hover:bg-red-600 cursor-pointer text-white text-xl font-medium text-center py-1 rounded-full mt-4" type="submit">Sign Up</button>
        </form>
